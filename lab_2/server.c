...
int sendFile(FILE *fd) {
    ...
    fseek(fd, 0, SEEK_SET);  // move to beginning of file
    //----------------------------------------------------------------
    ...
    while (ftell(fd) < filesize) {
        ...
        if (recvfrom(sockfd, &rcv_pkt, sizeof(rcv_pkt), 0, (struct sockaddr *)&client_info, (socklen_t *)&len) <= 0) {
            printf("\tTimeout! Resend packet seq %d!\n", snd_pkt.header.seq_num);
            snd_pkt.header.seq_num -= 1;
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        printf("%s %d %d\n", str, rcv_pkt.header.ack_num, rcv_pkt.header.is_last);
        //===============================================================
